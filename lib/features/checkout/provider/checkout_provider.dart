...
import 'package:ecommerce/features/checkout/domain/repo/checkout_repo.dart';
import 'package:ecommerce/features/offline_payment/domain/model/offline_payment_model.dart';
import 'package:ecommerce/features/cart/domain/models/shipping_method_model.dart';
import 'package:ecommerce/helper/api_checker.dart';
...
import 'package:ecommerce/features/checkout/view/digital_payment_order_place.dart';
...
class CheckOutProvider with ChangeNotifier {
  ...
  Future<void> placeOrder(
    ...
    isfOffline?
    apiResponse = await checkOutRepo.offlinePaymentPlaceOrder(addressID, couponCode,couponAmount, billingAddressId, orderNote, keyList, inputValueList, offlineMethodSelectedId, offlineMethodSelectedName, paymentNote):
    wallet?
    apiResponse = await checkOutRepo.walletPaymentPlaceOrder(addressID, couponCode,couponAmount, billingAddressId, orderNote):
    apiResponse = await checkOutRepo.placeOrder(addressID, couponCode,couponAmount, billingAddressId, orderNote);
  ...
  Future<ApiResponse> digitalPayment({String? orderNote, String? customerId,
    ...
    ApiResponse apiResponse = await checkOutRepo.digitalPayment(orderNote, customerId, addressId, billingAddressId, couponCode, couponDiscount, paymentMethod);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      _isLoading = false;
