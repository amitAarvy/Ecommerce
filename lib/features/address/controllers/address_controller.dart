...
import 'package:ecommerce/features/address/domain/repo/address_repository.dart';
...
class AddressController with ChangeNotifier {
  ...
  Future<ResponseModel?> getRestrictedDeliveryCountryList(BuildContext context) async {
    ...
    ApiResponse apiResponse = await addressRepo!.getDeliveryRestrictedCountryList();
  ...
  Future<void> getDeliveryRestrictedZipBySearch(BuildContext context, String searchName) async {
    _restrictedZipList = [];
    ApiResponse response = await addressRepo!.getDeliveryRestrictedZipBySearch(searchName);
  ...
  Future<void> getDeliveryRestrictedCountryBySearch(BuildContext context, String searchName) async {
    _restrictedCountryList = [];
    ApiResponse response = await addressRepo!.getDeliveryRestrictedCountryBySearch(searchName);
    if(response.response!.statusCode == 200) {
      _restrictedCountryList = [];
  ...
  Future<ResponseModel> updateAddress(BuildContext context, {required AddressModel addressModel, int? addressId}) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await addressRepo!.updateAddress(addressModel, addressId);
    _isLoading = false;
    ResponseModel responseModel;
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      Map map = apiResponse.response!.data;
